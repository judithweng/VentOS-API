...
#             p = PIRCS(com=fcom, par=fpar, int=fint, mod=fmod, val=fval)
#             p.save()
#             succeed = True
...
#             prev_session = Session.objects.all().filter(id=sid)
#             prev_session = prev_session[0]
#             prev_timestamp = prev_session.ventilatorState.timestamp

#             print("need to advance: " + str(ms-prev_timestamp) + " ms.")

#             ventilator_status = ventilator.advance(
#                 advance_time=ms-prev_timestamp, pressure_mouth=prev_session.patientState.pressure_mouth)
#             patient_status = patient.advance(
#                 advance_time=ms-prev_timestamp, pressure_mouth=prev_session.ventilatorState.pressure_mouth)
